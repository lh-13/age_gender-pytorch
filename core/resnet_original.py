...
class BasicBlock1(nn.Moudle):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(in_channle, out_channel, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2d(out_channle)

        self.conv3 = nn.Conv2d(out_channle, out_channle*expansion, kernel_size=1, stride=1, padding=1)
        self.bn3 = nn.BatchNorm2d(out_channle*expansion)
        self.stride = stride 
        self.downsample = downsample 
...
class Resnet(nn.Module):
    def __init__(self, block, layers, num_classes, grayflag=False):
        ...
        self.avgpool = nn.AvgPool2d(7, stride=1)     #kernel_size=7   why  因为到这里的时候输入size为[b, ]
...
def resnet50(num_class):
    model = Resnet(BasicBlock1, layers=[3, 4, 6, 3], num_classes=num_class, grayflag=0)
    return model 

def resnet101(num_class):
    model = Resnet(BasicBlock1, layers=[3, 4, 23, 3], num_classes=num_class, grayflag=0)
    return model 

def resnet152(num_class):
    model = Resnet(BasicBlock1, layers=[3, 8, 36, 3], num_classes=num_class, grayflag=0)
    return model 
